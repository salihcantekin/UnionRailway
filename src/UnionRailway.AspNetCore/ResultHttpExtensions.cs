...
public static class ResultHttpExtensions
{
    ...
    /// When a <paramref name="cancellationToken"/> is provided, cancellation is
    /// checked before awaiting the result task.
    /// </summary>
    ...
    /// <list type="bullet">
    ///   <item>Success with <see cref="Unit"/> → <c>204 No Content</c>.</item>
    ///   <item>Success → <c>200 OK</c>, or <c>201 Created</c> when <paramref name="createdUri"/> is supplied.</item>
    ///   <item>Error → delegates to <see cref="ToHttpResult(UnionError, Action{ProblemDetails}?)"/>.</item>
    /// </list>
    /// </summary>
    ...
    ///   <item><see cref="UnionError.Custom"/>        → Custom status code (default 422)</item>
    /// </list>
    /// </summary>
    /// <param name="error">The error to translate.</param>
    /// <param name="configureProblem">
    /// Optional callback to post-process the <see cref="ProblemDetails"/> before it is returned.
    /// Use this to add trace IDs, strip detail in production, or enrich extensions.
    /// </param>
